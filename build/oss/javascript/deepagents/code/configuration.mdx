...
Deep Agents Code stores configuration under `~/.deepagents/` and in project-level dotfiles. For the full directory tree, session storage, and skill paths, see [Data locations](/oss/javascript/deepagents/code/configuration#data-locations).

The main config files are:
<CardGroup cols={2}>
    <Card title="Config file" icon="file-code" href="/oss/javascript/deepagents/code/config-file">
        Edit `config.toml` for model defaults, provider settings, themes, and update settings.
    </Card>
    <Card title="Environment variables" icon="variable" href="/oss/javascript/deepagents/code/configuration#environment-variables">
        Set global API keys and secrets in `~/.deepagents/.env` or shell exports.
    </Card>
    <Card title="Hooks" icon="webhook" href="/oss/javascript/deepagents/code/hooks">
        Subscribe external commands to lifecycle events in `hooks.json`.
    </Card>
    <Card title="MCP servers" icon="plug" href="/oss/javascript/deepagents/code/mcp-tools">
...
**Provider API keys** use a separate order. See [Key resolution order](/oss/javascript/deepagents/code/credentials#key-resolution-order).
...
**Provider endpoints** (`base_url`) resolve with their matching API key. See [Endpoints, keys, and gateways](/oss/javascript/deepagents/code/config-file#endpoints-keys-and-gateways).
...
All four commands accept `--json` for machine-readable output. For the full list of management subcommands, see [CLI reference](/oss/javascript/deepagents/code/cli-reference).
...
For provider keys specifically, see [Provider credentials](/oss/javascript/deepagents/code/credentials).
...
By default, when Deep Agents Code loads skills it validates that a resolved skill file path stays inside one of the standard [skill directories](/oss/javascript/deepagents/code/configuration#skills). This prevents symlinks inside skill directories from reading arbitrary files outside those roots.
...
For user-defined themes, built-in overrides, and terminal-specific mappings, see the `[themes.*]` and `[ui.terminal_themes]` sections in [Config file](/oss/javascript/deepagents/code/config-file) or configure them directly in `config.toml`:
...
To route every user's model traffic through a managed gateway (provisioning a gateway key and base URL fleet-wide), see [Managed gateways](/oss/javascript/deepagents/code/config-file#managed-gateways).
...
<ResponseField name="DEEPAGENTS_CODE_EXPERIMENTAL" type="string" post={["optional"]}>
    Enable experimental features when set to a truthy value. [Plugins and marketplaces](/oss/javascript/deepagents/code/plugins) currently require this flag.
</ResponseField>

<ResponseField name="DEEPAGENTS_CODE_LANGSMITH_PROJECT" type="string" post={["optional"]}>
    Override the LangSmith project name for Deep Agents Code's own agent traces. Shell commands still run with the user's original `LANGSMITH_PROJECT`, so app, test, or script traces can appear in a separate project. See [Trace with LangSmith](/oss/javascript/deepagents/code/quickstart#trace-with-langsmith).
</ResponseField>

<ResponseField name="DEEPAGENTS_CODE_LANGSMITH_REPLICA_PROJECTS" type="string" post={["optional"]}>
    A second LangSmith project to *also* write agent traces to. When set and tracing is active, each agent run is dual-written to the primary project (from `DEEPAGENTS_CODE_LANGSMITH_PROJECT`, or `deepagents-code` by default) and this project. Off by default. See [Trace with LangSmith](/oss/javascript/deepagents/code/quickstart#trace-with-langsmith).
</ResponseField>
...
- [Provider credentials](/oss/javascript/deepagents/code/credentials)
- [Config file](/oss/javascript/deepagents/code/config-file)
- [CLI reference](/oss/javascript/deepagents/code/cli-reference)
- [Hooks](/oss/javascript/deepagents/code/hooks)
