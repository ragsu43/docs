...
<Steps>
    ...
    <Step title="Add provider credentials" icon="key">
        ...
        <Note>
            Web search uses [Tavily](https://tavily.com). Add a key from `/auth` or set `TAVILY_API_KEY`. See [Enable web search](/oss/javascript/deepagents/code/configuration#enable-web-search-with-tavily).
        </Note>
    </Step>
...
* <Icon icon="cloud" size={16} /> **[Remote sandboxes](/oss/javascript/deepagents/code/remote-sandboxes)** - run agent tools remotely instead of on your local machine.
* <Icon icon="search" size={16} /> **Web search** - search the web for up-to-date information and documentation. Requires a [Tavily API key](/oss/javascript/deepagents/code/configuration#enable-web-search-with-tavily).
* <Icon icon="list-check" size={16} /> **Task planning and tracking** - break down complex tasks into discrete steps and track progress.
* <Icon icon="target-arrow" size={16} /> **[Goals and rubrics](/oss/javascript/deepagents/code/goals-and-rubrics)** - define measurable objectives or grading criteria so the agent can check whether work is done.
* <Icon icon="users" size={16} /> **[Subagents](/oss/javascript/deepagents/code/subagents)** - delegate work to task-specific subagents.
* <Icon icon="brain" size={16} /> **[Memory storage and retrieval](/oss/javascript/deepagents/code/memory-and-skills#memory)** - store and retrieve information across sessions, enabling agents to remember project conventions and learned patterns.
...
* <Icon icon="puzzle" size={16} /> **[Skills](/oss/javascript/deepagents/code/memory-and-skills#skills)** - extend agent capabilities with custom expertise and instructions.
* <Icon icon="plug" size={16} /> **[MCP tools](/oss/javascript/deepagents/code/mcp-tools)** - load external tools from [Model Context Protocol](https://modelcontextprotocol.io/) servers.
* <Icon icon="chart-dots" size={16} /> **[Tracing](/oss/javascript/deepagents/code/overview#trace-with-langsmith)** - trace agent operations in LangSmith for observability and debugging.
...
<AccordionGroup>
    <Accordion title="Command-line options" icon="flag">
        ...
        | `--default-model [MODEL]` | Set the [default model](/oss/javascript/deepagents/code/providers#set-a-default-model) (omit `MODEL` to view the current default) |
        | `--clear-default-model` | Clear the [default model](/oss/javascript/deepagents/code/providers#set-a-default-model) |
        ...
        | `--install NAME`       | Install an optional extra (e.g., `quickjs`, `daytona`, `fireworks`), then exit. Add `--package` to treat `NAME` as a custom provider package installed via `uv --with` rather than an extra (see [arbitrary providers](/oss/javascript/deepagents/code/configuration#arbitrary-providers)), and `--yes` to skip confirmation prompts |
    ...
    <Accordion title="CLI commands" icon="terminal">
        ...
        | `dcode mcp login NAME [--mcp-config PATH]` | Run the OAuth login flow for an MCP server marked `auth: "oauth"`. See [MCP tools](/oss/javascript/deepagents/code/mcp-tools#oauth-login) |
        | `dcode mcp config`              | Show MCP config discovery paths        |
        | `dcode config show`             | Show every config option's effective value and the source it resolves from. See [Inspect configuration](/oss/javascript/deepagents/code/configuration#inspect-configuration) |
...
For the full reference—including `config.toml` schema, provider parameters, profile overrides, and hook configuration—see [Configuration](/oss/javascript/deepagents/code/configuration).
...
| `~/.deepagents/.env` | Global API keys and secrets. See [configuration](/oss/javascript/deepagents/code/configuration#environment-variables) |
| `~/.deepagents/hooks.json` | [Lifecycle event hooks](/oss/javascript/deepagents/code/configuration#hooks) (session start/end, task complete, etc.) |
...
<AccordionGroup>
    <Accordion title="Slash commands" icon="slash">
        ...
        - `/agents`: Hot-swap between pre-configured agents without relaunching. See [Command reference](/oss/javascript/deepagents/code/overview#command-reference) for details.
        - `/auth`: Manage stored API keys for model providers and services (such as Tavily web search). See [Provider credentials](/oss/javascript/deepagents/code/configuration#provider-credentials) for details.
        - `/goal <objective>`: Draft acceptance criteria from a measurable objective. See [Goals and rubrics](/oss/javascript/deepagents/code/goals-and-rubrics).
        - `/rubric`: Set explicit acceptance criteria for grading. See [Goals and rubrics](/oss/javascript/deepagents/code/goals-and-rubrics).
        ...
        - `/reload`: Re-read `.env` files, refresh configuration, and re-discover skills without restarting. Conversation state is preserved. See [`DEEPAGENTS_CODE_` prefix](/oss/javascript/deepagents/code/configuration#deepagents_code_-prefix) for override behavior.
        - `/theme`: Open the interactive theme selector to switch color themes. Built-in themes are available plus any [user-defined themes](/oss/javascript/deepagents/code/configuration#themes).
        ...
        - `/editor`: Open the current prompt in your external editor (`$VISUAL` / `$EDITOR`). See [External editor](/oss/javascript/deepagents/code/configuration#external-editor).
...
To override for a specific project, add the same keys to a `.env` in the project directory. See [environment variables](/oss/javascript/deepagents/code/configuration#environment-variables) for the full loading order.
...
<Accordion title="Separate agent traces from app traces">
    ...
    You can also scope LangSmith credentials to Deep Agents Code using the [`DEEPAGENTS_CODE_` prefix](/oss/javascript/deepagents/code/configuration#deepagents_code_-prefix) (e.g., `DEEPAGENTS_CODE_LANGSMITH_API_KEY`).
</Accordion>
