...
For an in-house or local provider you don't want to package, declare it under `[sandboxes.providers]` in `~/.deepagents/config.toml`. This parallels [arbitrary model providers](/oss/python/deepagents/code/config-file#arbitrary-providers) and uses the same `class_path` trust model.
...
<Warning>
    Setting `class_path` causes Deep Agents Code to import and run arbitrary Python from the named module—module-level code executes on import. This is the same trust model as the model [`class_path`](/oss/python/deepagents/code/config-file#arbitrary-providers): you control your own machine and your own config file.
</Warning>
