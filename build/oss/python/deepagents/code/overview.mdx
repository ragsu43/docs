...
<Steps>
    ...
    <Step title="Add provider credentials" icon="key">
        ...
        <Note>
            Web search uses [Tavily](https://tavily.com). Add a key from `/auth` or set `TAVILY_API_KEY`. See [Enable web search](/oss/python/deepagents/code/configuration#enable-web-search-with-tavily).
        </Note>
    </Step>
...
* <Icon icon="target-arrow" size={16} /> **[Goals and rubrics](/oss/python/deepagents/code/goals-and-rubrics)** - define measurable objectives or grading criteria so the agent can check whether work is done.
...
<AccordionGroup>
    <Accordion title="Command-line options" icon="flag">
        ...
        | `--default-model [MODEL]` | Set the [default model](/oss/python/deepagents/code/providers#set-a-default-model) (omit `MODEL` to view the current default) |
        | `--clear-default-model` | Clear the [default model](/oss/python/deepagents/code/providers#set-a-default-model) |
        ...
        | `--install NAME`       | Install an optional extra (e.g., `quickjs`, `daytona`, `fireworks`), then exit. Add `--package` to treat `NAME` as a custom provider package installed via `uv --with` rather than an extra (see [arbitrary providers](/oss/python/deepagents/code/configuration#arbitrary-providers)), and `--yes` to skip confirmation prompts |
    ...
    <Accordion title="CLI commands" icon="terminal">
        ...
        | `dcode config show`             | Show every config option's effective value and the source it resolves from. See [Inspect configuration](/oss/python/deepagents/code/configuration#inspect-configuration) |
...
<AccordionGroup>
    <Accordion title="Slash commands" icon="slash">
        ...
        - `/auth`: Manage stored API keys for model providers and services (such as Tavily web search). See [Provider credentials](/oss/python/deepagents/code/configuration#provider-credentials) for details.
