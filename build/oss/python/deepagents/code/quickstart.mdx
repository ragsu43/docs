...
<Steps>
    ...
    <Step title="Add provider credentials" icon="key">
        ...
        <Note>
            Web search uses [Tavily](https://tavily.com). Add a key with `/auth`. See [Enable web search](/oss/python/deepagents/code/credentials#enable-web-search-with-tavily).
        </Note>
    </Step>
...
<AccordionGroup>
    <Accordion title="Slash commands" icon="slash">
        ...
        - `/agents`: Hot-swap between pre-configured agents without relaunching. See [Command reference](/oss/python/deepagents/code/cli-reference#command-line-options) for related flags.
        - `/auth`: Manage stored API keys for model providers and services (such as Tavily web search). See [Provider credentials](/oss/python/deepagents/code/credentials) for details.
        - `/goal <objective>`: Draft acceptance criteria from a measurable objective. See [Goals and rubrics](/oss/python/deepagents/code/goals-and-rubrics).
        - `/rubric`: Set explicit acceptance criteria for grading. See [Goals and rubrics](/oss/python/deepagents/code/goals-and-rubrics).
        ...
        - `/plugins`: Manage [plugins and marketplaces](/oss/python/deepagents/code/plugins). Experimental; requires `DEEPAGENTS_CODE_EXPERIMENTAL=1`.
        - `/notifications`: Configure startup warning preferences.
        - `/reload`: Re-read `.env` files, refresh configuration, and re-discover skills without restarting. When experimental plugins are enabled, also reloads plugin skills and plugin MCP servers. Conversation state is preserved. See [`DEEPAGENTS_CODE_` prefix](/oss/python/deepagents/code/configuration#deepagents_code_-prefix) for override behavior.
...
Run `/auth` and add your LangSmith API key. Tracing is enabled on the next launch and persists across sessions. See [Provider credentials](/oss/python/deepagents/code/credentials#use-%2Fauth-recommended) for details on the credential manager.
...
- [Memory and skills](/oss/python/deepagents/code/memory-and-skills)
